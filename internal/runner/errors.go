...
//
// Правила «код единственной упавшей команды» здесь быть не может — по крайней
// мере в режиме fail-fast: там отказ одной цепочки останавливает соседние, и их
// собственный отказ маскируется отменой. Сколько команд «упало» — результат
// гонки, а не свойство конфигурации, и такое правило возвращало бы на одном и
// том же файле то один код, то другой.
//
// В режиме keep-going маскировки нет: отказы доезжают все, и «первый в порядке
// объявления» становится честным правилом, дающим один и тот же код от запуска
// к запуску.

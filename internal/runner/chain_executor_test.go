...
func (f *fakeRunner) Execute(_ context.Context, _ *flow.CommandChain, _ flow.Command) error {
	atomic.AddInt32(&f.count, 1)

	return nil
}

func (f *fakeRunner) ExecuteWithPipe(_ context.Context, _ *flow.CommandChain, _ flow.Command) error {
	atomic.AddInt32(&f.count, 1)
...
func TestChainExecutor_ExecuteParallel(t *testing.T) {
	...
	chain2 := flow.CommandChain{Name: "c2"}
	chain2.Add(flow.Command{Cmd: "echo", Pipe: true})

	if err := exec.ExecuteParallel(t.Context(), []*flow.CommandChain{&chain1, &chain2}); err != nil {
...
func TestChainExecutor_CancelContextStopsExecution(t *testing.T) {
	...
	_ = exec.ExecuteParallel(ctx, []*flow.CommandChain{&chain})
...
func TestChainExecutor_SkipsDisabledCommands(t *testing.T) {
	...
	chain.Add(flow.Command{Name: "will-run", Cmd: "echo"})

	if err := exec.ExecuteParallel(t.Context(), []*flow.CommandChain{&chain}); err != nil {
		t.Fatalf("ExecuteParallel returned error: %v", err)
	}
...
func (f *fakeRunnerConcurrent) Execute(_ context.Context, _ *flow.CommandChain, _ flow.Command) error {
...
func (f *fakeRunnerConcurrent) ExecuteWithPipe(ctx context.Context, _ *flow.CommandChain, command flow.Command) error {
...
func TestChainExecutor_PipeConcurrentWithinChain(t *testing.T) {
	...
	chain.Add(flow.Command{Name: "normal", Cmd: "echo"})
	...
	if atomic.LoadInt32(&runner.pipedStarted) != 1 || atomic.LoadInt32(&runner.normalStarted) != 1 {
		t.Fatalf("expected both commands to start, got piped=%d normal=%d",
			atomic.LoadInt32(&runner.pipedStarted), atomic.LoadInt32(&runner.normalStarted))
	}
}
...
func TestChainExecutor_WaitsForAllPiped(t *testing.T) {
	...
	chain.Add(flow.Command{Name: "p2", Cmd: "echo", Pipe: true})

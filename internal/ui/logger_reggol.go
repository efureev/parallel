...
// Writer оборачивается в reggol.SyncWriter — приложение пишет в один дескриптор
// из множества горутин (по две на каждую piped-команду), и без сериализации
// длинные строки разных цепочек накладываются друг на друга (находка P4).
func NewLogger(out io.Writer) Logger {
	// SyncWriter обязателен: приложение пишет в один дескриптор из множества
	// горутин (по две на каждую piped-команду), и без сериализации длинные
	// строки разных цепочек накладываются друг на друга (находка P4).
...
//
// До перехода на reggol v1 этот вопрос решался неявно: TextStyle.Wrap спрашивал
// глобальный флаг архивной зависимости gh.tarampamp.am/colors. Зависимости больше
// нет, поэтому проверка живёт здесь — иначе ANSI-последовательности попадали бы
// в файл при `parallel -f … > out.txt`.

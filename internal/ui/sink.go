...
//
// Сериализацию записи обеспечивает SyncWriter самой библиотеки логирования,
// а буферизации с управляемым сбросом она не предоставляет: без Sink каждая
// строка лога уходила бы отдельным системным вызовом write(2), и на потоке в
// сотни тысяч строк в секунду это доминирующая статья расходов.
//
...
// вовсе — поведение получалось само собой из отсутствия буфера.
type Sink struct {
	mu     sync.Mutex
	buf    *bufio.Writer
	out    io.Writer
